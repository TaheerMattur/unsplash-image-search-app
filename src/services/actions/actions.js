...
export const fetchedImages = (token) => {
  return function (dispatch) {
    axios
      .get(
        `https://api.unsplash.com/search/photos?client_id=3IvfH2FWSe4PZws1QOxsLtBRpdAlVeGWvuvH3CF2sHc&query=${token}&orientation=landscape&per_page=8&page=1`
      )
      .then((response) => {
...
export const loadMoreImages = (text, pageno) => {
  return function (dispatch) {
    axios
      .get(
        `https://api.unsplash.com/search/photos?client_id=3IvfH2FWSe4PZws1QOxsLtBRpdAlVeGWvuvH3CF2sHc&query=${text}&orientation=landscape&per_page=8&page=${pageno}`
      )
      .then((response) => {
        const images = response.data.results;
